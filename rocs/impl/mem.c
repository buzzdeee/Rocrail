...
static const char* _mem_getLastOperation( void ) {
  ...
  if( mt.type == MEMTYPE_ALLOC )
    ...
    op = "realloc";
  sprintf( __opStr, ">>>>> memLastOp: op=%s p=%p file=%s line=%d <<<<<", op, mt.p, mt.file, mt.line );
  return __opStr;
}
...
static Boolean __isMemValid( char* p, const char* file, int line, long* size, int id ) {
  if( p != NULL ) {
    ...
    if( memcmp( m->magic, __magic, MAGIC_SIZE ) != 0 ) {
      printf( ">>>>> Unknown memory block( %p ) %s:%d <<<<<\n", m, file, line );
      return False;
    }
    else if( m->id != id ) {
...
static void* __mem_realloc_magic( char* p, long newsize, const char* file, int line ) {
  if( p != NULL ) {
    ...
    printf( ">>>>> realloc( %p, %ld ) with NULL pointer! %s:%d <<<<<\n", p, newsize, file, line );
...
static void* _mem_allocTID( long size, int id, const char* file, int line ) {
  ...
  if( id == -1 ) {
    printf( " allicIDMem( %p, %ld ) %s line=%d: id -1 not allowed!!!\n", p, size, file, line );
  }
  /*if( m_bDebug )
...
static void* _mem_realloc( void* p, long size, const char* file, int line ) {
  ...
  if( mp == NULL ) {
    printf( "__mem_realloc_magic(%p, %ld) failed!", p, size );
  }
  /*if( m_bDebug )
...
static void _mem_free( void* p, const char* file, int line ) {
  ...
  if( m_bDebug )
    printf( " freeMem( %p) %s line=%d\n", p, file, line );
}

static void _mem_freeTID( void* p, int id, const char* file, int line ) {
  if( id == -1 && p!= NULL ) {
    printf( " freeIDMem( %p ) %s line=%d: id -1 not allowed!!!\n", p, file, line );
  }
  /*if( m_bDebug )
